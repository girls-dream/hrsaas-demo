<template>
  <div class="user-info">
    <i @click="$router.push(`/employees/printer/${userId}?type=personal`)" class="el-icon-printer"></i>
    <!-- 个人信息 -->
    ...
    <el-form label-width="220px">
      ...
      <div class="block">
        ...
        <el-row class="inline-info" type="flex" justify="center">
          <el-col :span="12">
            <el-button type="primary" @click="onSaveEmpoyeesInfo"
              >保存更新</el-button
            >
            <el-button @click="$router.back()">返回</el-button>
          </el-col>
...
export default {
  ...
  created() {
    this.loadUserDatil()
    this.loadPersonDatil()
  },
  methods: {
    // 加载上半部分
    async loadUserDatil() {
      this.userInfo = await userAllInfo(this.$route.params.id)
      this.$refs.headerImg.fileList.push({
        url: this.userInfo.staffPhoto,
      })
    },
    // 加载下半部分
    async loadPersonDatil() {
    ...
    async onSaveUserdetail() {
      if (this.$refs.headerImg.loading) {
        return this.$message.error('正在上传头像')
      }
      await saveUserDetailById(this.userInfo)
      this.$message.success('更新成功')
    },
    async onSaveEmpoyeesInfo() {
      if (this.$refs.employeesPic.loading) {
        return this.$message.error('正在上传照片')
      }
      //  调用父组件
